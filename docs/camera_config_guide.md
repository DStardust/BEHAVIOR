...
6. 每台相机还必须通过内容质量门：从所在房间体积较大的非结构物体和任务物中
   选取有限候选，使用相机视锥投影和 PhysX 遮挡射线确认至少一个物体可见，且总投影
   面积不少于 1024 像素。门、窗、开关、地毯和墙饰不能单独作为补充视角的内容证据；
   但它们仍可在对应任务中作为任务目标通过覆盖检查。这个门不会用 marker，也不会
   移动原场景物品；失败时只丢弃该相机候选。相机记录中的 `visible_scene_content`、`scene_content_pixel_count` 和
   `scene_content_quality_ok` 用于审计与问题定位。

生成结果在 `validation.camera_coverage` 中记录相机数量、目标逐物体覆盖次数、机器人覆盖次数、是否请求及达成重复覆盖，以及逐相机内容质量证据。`audit_deltasg_outputs.py` 会拒绝少于 2 个或多于 3 个全局相机、目标未被全局相机覆盖、没有全局相机看到机器人，或内容质量字段明确失败的样本。
